...
head -n 1 test_results.txt | grep -q "200" && echo -e "${GRE} Status is 200 OK${RES}" || (echo -e "${RED}Status is not ok${RES}" && break)
echo ""
...
head -n 1 test_results.txt | grep -q "302" && echo -e "${GRE} Status is 302 found" || (echo -e "${RED}Status is not ok${RES}" && break)
echo ""
...
curl -s --http1.1 -X DELETE localhost:$PORT/text.txt -i -o test_results.txt
head -n 1 test_results.txt | grep -q "404" && echo -e "${GRE} Status is 404 OK (not found)" || (echo -e "${RED}Status is not ok${RES}" && break)
echo ""
...
head -n 1 test_results.txt | grep -q "404" && echo -e "${GRE} Status is 404 OK${RES}" || (echo -e "${RED}Status is not ok${RES}" && break)
echo ""
...
head -n 1 test_results.txt | grep -q "400" && echo -e "${GRE} Status is 400 OK${RES}" || (echo -e "${RED}Status is not ok${RES}" && break)
echo ""
